 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few positive numbers") {
    ...
    assert(sum(List(8,10,2)) === 20)
  }

  test("max of a few positive numbers") {
    assert(max(List(3, 7, 2)) === 7)
    assert(max(List(10, 6, 4)) === 10)
    assert(max(List(3, 7, 20)) === 20)
  }
  ...
  test("max of a few negative numbers") {
    ...
    assert(max(List(-10, -6, -4)) === -4)
    assert(max(List(-3, -7, -20)) === -3)
  }

  test("sum of negative and positive numbers"){
    assert(sum(List(-3, 7, -2)) === 2)
    assert(sum(List(3,-2,-2)) === -1)
  }

  test("max of negative and positive numbers"){
    assert(max(List(-3, 7, -2)) === 7)
    assert(max(List(3,-2,-7)) === 3)
  }

  test("max of repetitive numbers"){
